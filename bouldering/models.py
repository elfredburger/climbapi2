...
class BoulderSector(models.Model):
    ...
    sector_location=models.ForeignKey(BoulderLocation, verbose_name='sector location',on_delete=models.SET_DEFAULT,default=1,blank=True)
    sector_coords=models.CharField(max_length=200,verbose_name='sector coordinates from google maps', default=1,blank=True )
...
class BoulderPhoto(models.Model):
    ...
    photo =  models.ImageField(upload_to='static/boulders/images/',  blank=True,verbose_name="Фото")
...
class Boulder(models.Model):
    ...
    boulder_finder=models.ForeignKey(User,blank=True,blank=True,verbose_name='boulder finder',on_delete=models.SET_DEFAULT,default=1)
    ...
    boulder_sector=models.ForeignKey(BoulderSector,blank=True,verbose_name='sector where boulder is located',on_delete=models.SET_DEFAULT,default=1)
    boulder_coords = models.URLField(max_length=1000, blank=True, verbose_name='Ссылка на локацию трассы', help_text='Принимаются метки с гугл карт', blank=True)
    boulder_photos=models.ManyToManyField(BoulderPhoto)
    boulder_info=models.CharField(max_length=300,blank=True,blank=True,verbose_name='information about a boulder',default='no information')
